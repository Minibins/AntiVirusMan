...
public class Contacter : MonoBehaviour
{
    ...
    {
        if ((contactType == ContactType.Collision | contactType == ContactType.Both) &&
            (contacterLayer == collision.gameObject.layer | contacterTag == collision.gameObject.tag))
        {
    ...
    {
        if ((contactType == ContactType.Trigger | contactType == ContactType.Both) &&
            (contacterLayer == collision.gameObject.layer | contacterTag == collision.gameObject.tag))
        {
            methodArgument = collision;
            onContact.Invoke();
